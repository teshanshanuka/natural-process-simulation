...
"""
Creates images in dirs named with `Re` value
To generate videos,

```
cd <val>
ffmpeg -r 12 -i vel.%04d.png -q:v 0  ../<val>.avi
```
"""

for Re in [30]:
# for Re in [5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0, 45.0, 50.0]:
    odir = str(Re)
    os.mkdir(odir)
    ###### Flow definition #########################################################
    ...
    for time in range(maxIter):
        ...
        if (time%100==0):
            img = sqrt(u[0]**2+u[1]**2).transpose()
            plt.clf()
            plt.imshow(img, cmap=cm.Reds)
            plt.savefig(os.path.join(odir, "vel.{0:04d}.png".format(time//100)))
